...
%\VignetteKeywords{Management and visualization the network of functional interaction between proteins.}
...
Results can be shown visually and managed with different browsers cross-platform. 
...
\section{Data}
As an example, we have generated protein-protein interaction data for several species from the PINA database \citep{Cowley12, Wu09} and the STRING database \citep{Szklarczyk11}.
...
In this package four methods are presented to format PPI data downloaded from PINA and STRING databases. 
These methods are {\tt getMappingFile}, {\tt formatSTRINGPPI}, {\tt formatPINAPPI} and {\tt combinePPI}.
In this package four other methods are also used to manage, visualize and edit protein-protein interaction networks. 
...
<<example1,echo=TRUE,print=FALSE>>=
  library(cisPath)
  input <- system.file("extdata", "uniprot_sprot_human10.dat", 
                                            package="cisPath")
  mappingFile <- file.path(tempdir(), "mappingFile.txt")
  getMappingFile(input, output=mappingFile)
@
\end{center} 
...
The method {\tt combinePPI} is used to combine the PPI information that is generated from different databases.
\begin{center}
<<example4,echo=TRUE,print=FALSE>>=
  output <- file.path(tempdir(), "STRINGANDPINA.txt")
  combinePPI(c(PINAPPI, STRINGPPI), output)
@
\end{center} 

We processed the PPI data from the PINA and STRING databases. 
Users can access these result files from our website \url{http://www.isb.pku.edu.cn/cispath/}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Examples}
\subsection{Example of the networkView method}
This method is used to visualize the input proteins in the PPI network, 
and may be used to view the evidence which supports the protein interactions. 
If the PPI information downloaded from PINA and/or STRING is used, 
the PubMed ID for corresponding paper and/or STRING score will also be shown. 
If the PubMed ID is provided, users can access the paper directly using the link. 
Sometimes two proteins cannot interact directly, but can be mediated by scaffold proteins. 
As such, this tool can also display other proteins which can interact with at least two input proteins. 
In addition, with this method, users can choose the visual style (color and model size) in which the proteins will be displayed.
...
The small node (PTEN) will be displayed as a smaller circle in another default color. 
All node colors can be customized by the parameter {\tt nodeColors} in this method. 
The other proteins which can interact with at least two given main node proteins are presented as leaf nodes. 
The color of these leaf nodes can be customized with the parameter {\tt leafColor}.
...
To identify the paths utilizing the shortest steps (instead of minimal cost), the parameter {\tt byStep} should be set as {\tt TRUE}. 
In this situation, all the edge cost will be assigned as {\tt 1}. 
Setting this parameter as {\tt TRUE}, allows users to view more possible paths between two proteins. 
...
The other proteins which can interact with at least two proteins in the shortest path are also presented as leaf nodes. 
...
The evidences for the interactions between the proteins along the shortest path are shown in the table below the graph. 
By clicking the {\tt View Graph} link in the table at the bottom, users can select the path to be shown graphically.

In order to avoid inputting invalid target protein names, the unique identifier Swiss-Prot accession numbers may alternately be used as input. 
...
<<example9,echo=TRUE,print=FALSE, results=hide>>=
  infoFile <- system.file("extdata", "PPI_Info.txt", package="cisPath")
  outputDir <- file.path(tempdir(), "TP53_example")
  results <- cisPath(infoFile, "TP53", outputDir)
  results["GH1"]
...
\begin{center}
<<example11,echo=TRUE,print=FALSE>>=
  name2protFile <- system.file("extdata", "name2prot.txt", package="cisPath")
...
With combined PPI data from PINA and STRING, the output from a single source protein is about {\tt 1.2G} in size.
...
In this method, the output HTML file is a network editor. 
