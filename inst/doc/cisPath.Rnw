...
\section{Data}
As an example, we have generated PPI data for several species from the PINA database \citep{Cowley12, Wu09} and the STRING database \citep{Szklarczyk11, Franceschini13}.
...
Four methods have been included in this package to format the files downloaded from the PINA and STRING databases. 
...
We have processed the PPI data from the PINA and STRING databases. 
